...
class ScholarScraper():
    ...
    def scrape_topic (self, topic, min_len=0, max_len=9999):
        ...
        with open('loadings\\topic_papers\\{}.txt'.format(topic), 'a', encoding='utf-8') as file:
            for counter, pubblication in enumerate(keyword.publications):

                if len(pubblication.bib['title']) < min_len or len(pubblication.bib['title']) > max_len:
                    continue
    ...
    def scrape_author (self, author_name, min_len=0, max_len=9999):
        ...
        with open('loadings\\authors_papers\\{}.txt'.format(author_name), 'a', encoding='utf-8') as file:
